...
function changePassword(id, currentPwd, newPwd) {
  return Account.findOne({ _id: id })
    ...
    .then(hash => Account.findOneAndUpdate({ _id: id }, { password: hash }));
}
...
function deleteAccount(id, currentPwd) {
  return Account.findOne({ _id: id })
    ...
    .then(UserProfile.findOneAndDelete({ _id: id }));
}

function signToken(account) {
  const token = jwt.sign(
    { user_id: account._id, role: account.role },
...
function resetPassword(email) {
  return Account.findOne({ email: email }).then(account =>
    !account
...
function setTempPassword(token, tempPwd) {
  return bcrypt
    .hash(tempPwd, 8)
    .then(hash => Account.findByIdAndUpdate(token.user_id, { password: hash }))
    .then(account =>
      !account ? Promise.reject(new Error("Account error")) : account
    );
}
